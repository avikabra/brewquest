...
export async function GET(req: NextRequest) {
  try {
    ...
    try {
      ...
      if (feErr) {
        console.error('Friend edges query error (table may not exist):', feErr);
        return NextResponse.json({ items: [] }); // Return empty if no friends table
      }
      
      friendIds = Array.from(new Set(edges.map(e => e.user_id === userId ? e.friend_id : e.user_id)));
    } catch (friendError) {
      console.error('Friend edges table may not exist yet:', friendError);
      return NextResponse.json({ items: [] }); // Return empty if no friends table
    }
